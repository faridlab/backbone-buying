//! Domain policy for PurchaseAgreement aggregate
//!
...
/// Domain policy for PurchaseAgreement — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PurchaseAgreement>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PurchaseAgreementDomainPolicy = PermitAllPolicy<PurchaseAgreement>;
