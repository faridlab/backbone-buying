...
//!
//! The create gate resolves the currency-rate snapshot: a PO denominated in the home currency
//! (the settings row's denomination) fixes rate 1 regardless of what was supplied; any other
//! currency REQUIRES a positive rate and refuses loudly otherwise (`CurrencyRateRequired`) — a
//! silent rate-1 default would mis-classify every foreign-currency PO at the double-validation
//! gate.
//!
...
use super::buying_write_service::{
    is_dup, price_document, relay_ambient_scope, BuyingError, BuyingWriteService, NewPurchaseOrder,
};

/// Resolve the order-time currency-rate snapshot against the home currency (the settings row's
/// denomination; the pinned convention: HOME currency units per 1 PO-currency unit).
///
...
/// the double-validation gate converts the PO total into home currency with this same snapshot.
...
impl BuyingWriteService {
    ...
    pub async fn create_purchase_order(&self, o: NewPurchaseOrder) -> Result<Uuid, BuyingError> {
        ...
        // The create gate: resolve the home currency (settings row, schema default IDR), then the
        // rate snapshot. The read is id-only (there is at most one live settings row per scope);
        // the composed decorator owns visibility.
