...
metaphor migration generate Invoice buying
metaphor migration run
...
(`buying` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed buying          # run Rust seeders
metaphor migration generate-seeds buying  # emit SQL seed files
```
